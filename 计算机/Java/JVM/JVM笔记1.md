...
+ 主要包括四种验证，文件格式验证，元数据验证（进行语义分析，例如是否有父类、是否实现了父类或接口中需要实现的所有方法），字节码验证（确定语义是否合法、符合逻辑），符号引用验证（判断该类是否缺少或者被禁止访问它依赖的某些外部类、方法、字段等资源）。
...
+ 遇到new、getstatic、putstatic或invokestatic四条字节码时，如果类没有进行初始化，会触发初始化阶段，这四个指令的具体场景如下：
  + 使用new关键字实例化对象的时候。如果使用new关键字实例化对象数组并不会触发对应类的初始化阶段。
  + 读取或者设置一个类型的静态字段的时候。**被final修饰，已经在编译器把结果放入常量池的静态字段除外。**
  + 调佣一个类型的静态方法时。
+ 使用java.lang.reflect包的方法对类型进行反射调用的时候，如果类型没有进行过初始化，则需要先触发期初始化。
+ 当初始化类的时候，如果发现其父类还没有进行过初始化，则需要先触发期父类的初始化。
+ 当虚拟机启动时，用户需要指定一个要执行的主类（包含main方法的所在的类），虚拟机需要先先初始化这个主类。
+ 当一个接口中定义了JDK 8中新加入的默认方法，如果这个接口的实现类发生了初始化，那该接口在这个类初始化之前需要被初始化。
...
一个进程拥有一个JVM实例，一个JVM实例拥有一个运行时数据区，进程中的线程**共享运行时数据区中的堆和方法区**，每一个线程各自用用一套程序计数器、本地方法栈和虚拟机栈
